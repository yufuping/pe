...
def render_gravel_pebbles(
    ...
    # Higher density �� allow closer packing (real hex gravel is nearly edge-touching)
...
def render_ar_conc_aggregate(
    ...
    # Sparse hollow triangles �� key differentiator vs GRAVEL
    ...
    # No pebble-like closed polygons �? those leak into GRAVEL confusion.
    return img.resize((size, size), Image.Resampling.LANCZOS)
...
def render_ar_sand(
    ...
    """
    AR-SAND: sand / grit stipple only — no triangles (vs AR-CONC), no pebble loops.
    """
    rng = rng or np.random.default_rng()
    ss = 2
    S = size * ss
